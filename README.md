...
- **Coordenador de dados:** um [DataUpdateCoordinator](https://developers.home-assistant.io/docs/data_update_coordinator_index/) gerencia as chamadas periódicas à API (intervalo padrão de 5 minutos), tratando erros de conexão e renovação do token. A cada ciclo, a integração compara o timestamp `ultimaleitura` retornado pela API com o da consulta anterior; **os sensores só gravam um novo estado no Home Assistant quando o inversor publicou uma leitura mais recente**. Isso garante que `last_changed` e `last_updated` reflitam o momento real da coleta de dados pelo inversor, e não o instante arbitrário em que o Home Assistant fez a requisição.
